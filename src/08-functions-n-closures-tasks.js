...
function getPolynom(...a) {
  const args = a.reverse();
  return function polynom(x) {
    return args.map((arg, index) => {
      if (index === 1) return arg * x;
      if (index === 2) return arg * (x ** 2);
      return arg;
...
function logger(func, logFunc) {
  return function f(...args) {
    const argsToString = args.map((arg) => (typeof arg === 'object' ? JSON.stringify(arg) : arg.toString()));
    const start = `${func.name}(${argsToString}) starts`;
    const end = `${func.name}(${argsToString}) ends`;
